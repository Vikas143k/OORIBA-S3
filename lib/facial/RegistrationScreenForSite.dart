...
class RegistrationScreen extends StatefulWidget {
  ...
  const RegistrationScreen({super.key, required String siteManagerId,required this.location});
...
class _HomePageState extends State<RegistrationScreen> {
  ...
  final FirebaseFirestore _db = FirebaseFirestore.instance;
      final FirestoreService retrieveEmployee = FirestoreService();
  late ImagePicker imagePicker;
  File? _image;
  // late String empLocation;
  late FaceDetector faceDetector;
  late Recognizer recognizer;
  // String emplocation=widget.location;
  List<Face> faces = [];
  var image;
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    // Check if Employee ID exists in SiteManagerAuth collection
                    String employeeId = textEditingController.text.trim();
                    DocumentSnapshot docSnapshot = await _db.collection('SiteManagerAuth').doc(employeeId).get();
                    DocumentSnapshot docSnapshot2 = await _db.collection('Regemp').doc(employeeId).get();
                  
                    if (docSnapshot.exists) {
                      setState(() {
                        errorMessage = "Employee ID already exists or registration";
                      });
                    } 
                      ...
                      // If Employee ID does not exist, proceed with the registration
                      await uploadEmployeeImage(employeeId, _image!);
